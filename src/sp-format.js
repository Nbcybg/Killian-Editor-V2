...
//
// [alpha.88 ข้อ 4] `firstPage:false` = มาตรฐานอุตสาหกรรม — **หน้า 1 ของบทไม่ใส่เลข**
// เลขเริ่มปรากฏที่หน้า 2 · เปิดเองได้ที่ ตั้งค่า → หน้ากระดาษ
// .82 เคยตั้งเป็น true เพราะตอนนั้น "หน้าแรกไม่ใส่เลข" ถูกตีความว่าหมายถึงหน้าปก —
// แต่หน้าปก/หน้ารายชื่อแยกเป็น "หน้าหน้าเล่ม" ที่ไม่ถูกนับตั้งแต่ .81r2 อยู่แล้ว
// กฎของ .82 ที่ยังอยู่ครบคือ **ฉากที่เริ่มหน้า 12 หน้าแรกของมันต้องได้ "12."**
// (ดู pageNumberLabel — เว้นเฉพาะตอนเลขที่จะพิมพ์เท่ากับ 1 จริง ๆ)
...
export function pageNumberLabel(index, fmt, startPage) {
  ...
  const n = start + i - 1;
  // [alpha.88 ข้อ 4] "ไม่ใส่เลขหน้าแรก" = ไม่ใส่ให้ **หน้า 1 ของบท** ไม่ใช่ "หน้าแรกของไฟล์"
  // กฎที่ผู้ใช้ตั้งไว้ตั้งแต่ .82 ยังอยู่ครบ: ฉากที่เริ่มหน้า 12 หน้าแรกของมันต้องได้ "12."
  if (n === 1 && !pn.firstPage) return '';
  return String(n) + (pn.suffix || '');
}
