...
async function restoreScene(p, info) {
  ...
  await mutateJson(kapi, sf, (d) => {                  // [alpha.156] อ่านสด-แก้-เขียนในคิว
...
async function restoreChapter(p, info) {
  ...
  // [alpha.156] เขียนกลับแบบอ่านสด — แตะเฉพาะบทนี้ (ระหว่างย้ายไฟล์ข้างบน งานอื่นอาจแก้สองไฟล์นี้ไปแล้ว)
  const entry = draft.chapters.find((c) => c.guid === info.ch.guid);
  const rowsNow = s2.chapters[info.ch.guid] || [];
  await mutateJson(kapi, df, (fresh) => {
    fresh.chapters = fresh.chapters || [];
    const i = fresh.chapters.findIndex((c) => c.guid === info.ch.guid);
    if (i >= 0) fresh.chapters[i] = entry; else fresh.chapters.push(entry);
  }, { fallback: { chapters: [] } });
  await mutateJson(kapi, sf, (fresh) => {
    fresh.chapters = fresh.chapters || {};
    const merged = [...(fresh.chapters[info.ch.guid] || [])];
    for (const r of rowsNow) if (!merged.some((x) => x.id === r.id)) merged.push(r);
    fresh.chapters[info.ch.guid] = merged;
  }, { fallback: { chapters: {} } });
  return note;
}
