...
/**
 * [alpha.162 · W1-4] เวลาประทับของชื่อในถังขยะ — **ห้ามซ้ำกับตัวที่แจกไปแล้ว**
 *
 * ต้นตอ: ทุกทางลบสร้างชื่อปลายทางเป็น `Date.now().toString(36) + '-' + ชื่อไฟล์` ตรง ๆ
 * การลบหลายฉากรวดเดียว (`deleteSceneSilent` วนลูป) ได้เวลาเดียวกันได้ และ **ทุกบทมี `scene-01.md`**
 * (กฎ alpha.159) → ชื่อในถังชนกัน แล้ว `kapi.move` = rename ทับของเดิมเงียบ ๆ พร้อมใบกู้คืน
 * = ฉากที่ลบก่อนหายถาวร · ตัวนี้บังคับให้เวลาเดินหน้าอย่างน้อย 1 ms ต่อการแจกหนึ่งครั้ง
 * @param {number} now เวลาปัจจุบัน · @param {number} last ค่าที่แจกไปแล้วล่าสุดในรอบนี้
 */
export function nextStamp(now, last) {
