...
contextBridge.exposeInMainWorld('kapi', {
  ...
  // [alpha.159] ที่อยู่ไฟล์ (.ttf/.otf) ของวงศ์แรกในรายการที่เครื่องมี — PDF ของบทใช้ฝังฟอนต์ไทย
  fontFile: call('fonts:file'),
  ...
  join: (...a) => {
    ...
    if (bad >= 0) {
      return Promise.reject(new Error(
        // [alpha.164 ข้อ B5] ข้อความถึงนักพัฒนา (throw) — preload ใช้ t() ไม่ได้ จึงเป็นอังกฤษ ไม่ใช่ไทยฮาร์ดโค้ด
        'kapi.join: path part #' + (bad + 1) + ' is not a string (' +
        (a[bad] === undefined ? 'undefined' : JSON.stringify(a[bad])) + ') — ' +
        'all parts: [' + a.map((x) => (typeof x === 'string' ? x : String(x))).join(' , ') + ']'));
    }
