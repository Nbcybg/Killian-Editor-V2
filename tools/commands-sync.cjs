...
// [alpha.154 ข้อ 6] ★ ผู้ใช้: *"icon อยู่ส่วน icon ภาษาอยู่ส่วนภาษา ... ไม่ซ้ำซ้อน · csv อันไหน header อันไหนไม่ใช้ ลบออกไป"*
// เดิมมี 13 ช่อง — ป้ายชื่อไทย/อังกฤษ (สำเนาของไฟล์ภาษา) · อยู่ตรงไหน · handler · หมายเหตุ ฯลฯ
// ซึ่งโปรแกรมไม่ได้อ่านเลยสักช่อง และเป็นข้อความที่ต้องแปลซ้ำอีกที่ · ตอนนี้เหลือเฉพาะที่โปรแกรมใช้จริง
// (ชื่อ/คำอธิบายของคำสั่งอยู่ในไฟล์ภาษาที่เดียว: ป้ายตามคีย์ของเมนู/ปุ่ม · คำอธิบาย `ui.tip.<command_id>`)
const COLUMNS = ['command_id', 'icon', 'shortcut'];
...
// ตัวอักษรนำหน้าป้ายเก่า → ชื่อไอคอน (ใช้เดาไอคอนของแถวใหม่เท่านั้น) — อ่านจาก glyphs.csv ที่โปรแกรมใช้จริง
const GLYPH_NAME = {};
for (const r of readCsvObjects(R('icons/glyphs.csv'))) if (r.glyph && r.name && !GLYPH_NAME[r.glyph]) GLYPH_NAME[r.glyph] = r.name;
...
const rows = [];
const missingInCsv = [];
const report = [];                       // ข้อสังเกตสำหรับคนดูแล — พิมพ์ออกจอ ไม่เขียนลง CSV
for (const c of CMDS.values()) {
  const old = base.get(c.id);
  if (!old) missingInCsv.push(c.id);
  const ch = c.id.split(':')[0];
  const isUi = c.id.startsWith('ui:');
  if (!isUi && !HANDLED.has(ch)) report.push(`${c.id}: ไม่มี case ใน handleCommand`);
  ...
  if (icon && !svgOk(icon) && !glyphOk.has(icon)) report.push(`${c.id}: ไม่มีทั้ง svg/${icon}.svg และตัวสำรอง`);
  if (!isUi && c.id.startsWith('toggle-panel:') && !shortcut && !panelSkip[c.id.slice(13)]) report.push(`${c.id}: แผงนี้ไม่มีคีย์ลัด`);
  rows.push({ command_id: c.id, icon, shortcut, _cat: c.cat });
}
// แถวที่ผู้ใช้มีแต่โค้ดไม่มีแล้ว — เก็บไว้ (ไม่ลบข้อมูลของผู้ใช้เงียบ ๆ) แต่บอกให้รู้
for (const [id, old] of base) if (!CMDS.has(id)) { rows.push({ ...old, _cat: 'zz' }); report.push(`${id}: ไม่พบคำสั่งนี้ในโค้ดแล้ว — ลบแถวได้ถ้าเลิกใช้จริง`); }
rows.sort((a, b) => (a._cat || 'zz').localeCompare(b._cat || 'zz') || a.command_id.localeCompare(b.command_id));

/** คีย์ภาษาที่เป็น "ป้ายชื่อของคำสั่ง" (เมนู · ปุ่ม · FAB · ตารางคีย์ลัด) — เทสใช้ตรวจว่าไม่มีคีย์ลัดฝังในป้าย */
const LABEL_KEYS = new Set([...CMDS.values()].flatMap((c) => c.labels.map((l) => l.key)));

module.exports = { rows, missingInCsv, HANDLED, LABEL_KEYS, COLUMNS, CMDS, tx };
...
if (require.main === module) {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (report.length && process.argv.includes('--report')) console.log('  · ' + report.join('\n  · '));
  }
}
