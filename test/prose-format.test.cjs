...
// ═══════ [alpha.81r ข้อ 5] Markdown → บล็อกนิยาย (ใช้จัดหน้าในช่องตัวอย่าง) ═══════
{
  ...
  // ★★ [alpha.132r] **บรรทัดว่างเป็นบล็อกแล้ว** — สัญญาข้อนี้กลับด้านจากของเดิม
  // เหตุผล: ช่องตัวอย่าง PDF ใช้ตัวนี้ ส่วนไฟล์จริงใช้ `mdToHtmlBody` ซึ่งเก็บบรรทัดว่างไว้
  // (alpha.132 ข้อ 1) → ถ้าที่นี่ยังทิ้ง ตัวอย่างจะสั้นกว่าไฟล์จริงและตัดหน้าคนละที่
  // รูปที่ใช้เป็นอันเดียวกับ `proseBlocksFromDoc`: ย่อหน้าว่าง = `{type:'p', text:''}`
  check('★★ แยกบล็อกได้ 7 ชิ้น — บรรทัดว่างนับเป็นย่อหน้าว่างด้วย',
        b.length === 7, b.length + ':' + b.map((x) => x.type).join(','));
  check('หัวข้อได้ type/level ถูก', b[0].type === 'h1' && b[0].level === 1 && b[0].text === 'หัวข้อ');
  check('★ บรรทัดว่างเป็นย่อหน้าว่าง (ไม่ใช่ชนิดใหม่ — ตัวนับ/ตัววาดเดิมรองรับอยู่แล้ว)',
        b[1].type === 'p' && b[1].text === '', JSON.stringify(b[1]));
  check('ย่อหน้าเป็น p', b[2].type === 'p' && b[2].text === 'ย่อหน้าแรก');
  check('ยกคำพูดถอด > ออก', b[3].type === 'blockquote' && b[3].text === 'ยกคำพูด');
  check('รายการถอดสัญลักษณ์ออก', b[4].type === 'li' && b[4].text === 'รายการ');
  check('เส้นคั่นเป็น hr', b[5].type === 'hr');
  ...
  // ══ ★★ [alpha.132r2] ข้อความในบล็อกต้องเป็น "อย่างที่ผู้อ่านจะเห็น" ไม่ใช่ซอร์สมาร์กดาวน์ ══
  // ผู้ใช้: *"pdf ส่งออกเป็น code markdown"* — ที่เห็นคือ **ช่องตัวอย่าง** ซึ่งวาดจากตัวนี้
  {
    const NL = String.fromCharCode(10);
    const d = P.mdToProseBlocks(['## หัวข้อ **เด่น**',
                                 'ย่อหน้า **หนา** *เอียง* ~~ฆ่า~~ _ขีด_',
                                 '- ข้อ [[โทระ|เขา]] พูด',
                                 '> ยกมา **เน้น**',
                                 '![คำบรรยาย](../Images/a.png)'].join(NL));
    check('★★ ไม่มีเครื่องหมายมาร์กดาวน์หลงเหลือในข้อความที่จะวาด',
          d.every((b) => !/\*\*|~~|\[\[/.test(b.text || '')),
          JSON.stringify(d.map((b) => b.text)));
    check('★ หัวข้อถอดเครื่องหมายแล้วแต่ข้อความยังครบ',
          d[0].type === 'h2' && d[0].text === 'หัวข้อ เด่น', d[0].text);
    check('★ ลิงก์เอนทิตี้เหลือเฉพาะข้อความที่แสดง',
          d[2].type === 'li' && d[2].text === 'ข้อ เขา พูด', d[2].text);
    check('★★ บรรทัดที่เป็นรูปล้วนกลายเป็นบล็อกรูป (ไม่ใช่ย่อหน้าที่มีโค้ด ![…](…))',
          d[4].type === 'figure' && d[4].alt === 'คำบรรยาย' && !/!\[/.test(d[4].text || ''),
          d[4].type + ':' + (d[4].text || ''));
    // ★ สแปนสีต้องรอด — ตัววาดของช่องตัวอย่างเปลี่ยนมันเป็นสีจริงต่อ (alpha.132r)
    const c = P.mdToProseBlocks('ปกติ <span style="color:#b03030">แดง</span> ท้าย');
    check('★★ สแปนสีไม่ถูกถอดทิ้งไปกับเครื่องหมายอื่น',
          c[0].text.includes('<span style="color:#b03030">แดง</span>'), c[0].text);
  }
  // ══ ★★ [alpha.132r4] ข้อที่ว่างเปล่าก็เป็นข้อ — บรรทัดว่างในรายการต้องไม่หาย ══
  // ผู้ใช้: *"บรรทัดว่างโดนลบ ในกรณีที่ใช้ตัวเลขหรือ bullet แล้วกด enter ลงมา"*
  // (`- ` / `12. ` ถูกตัดวรรคท้ายไปแล้วเหลือ `-` / `12.` — กฎเดิมบังคับว่าต้องมีวรรคตาม)
  {
    const NL = String.fromCharCode(10);
    const e = P.mdToProseBlocks(['1. ก', '2.', '3. ข', '- ค', '-', '- ง'].join(NL));
    check('★★ ข้อว่างในรายการเลขนับเป็นบล็อก (ไม่กลายเป็นย่อหน้าที่มีข้อความ "2.")',
          e.length === 6 && e.every((b) => b.type === 'li'),
          JSON.stringify(e.map((b) => b.type + ':' + b.text)));
    check('★ ข้อว่างมีข้อความว่างจริง ไม่ใช่เลขข้อค้างอยู่',
          e[1].text === '' && e[4].text === '', JSON.stringify([e[1].text, e[4].text]));
    check('★ ข้อว่างกินที่หนึ่งบรรทัด (ไม่ใช่ 0)',
          P.proseBlockLines({ type: 'li', text: '' }, P.mergeProseFormat({}), 60) >= 1);
  }
  // ★ ย่อหน้าว่างหัว/ท้ายเป็นเศษจากการประกอบ ไม่ใช่ช่องไฟที่ผู้เขียนตั้งใจ (กฎเดียวกับ mdToHtmlBody)
  check('★ ย่อหน้าว่างหัว/ท้ายถูกตัดทิ้ง',
        P.mdToProseBlocks(String.fromCharCode(10, 10) + 'ก' + String.fromCharCode(10, 10)).length === 1,
        JSON.stringify(P.mdToProseBlocks(String.fromCharCode(10, 10) + 'ก' + String.fromCharCode(10, 10))));
  // ★★ ต้องนับบรรทัดได้เท่ากับย่อหน้าหนึ่งบรรทัด (ไม่งั้นตัวอย่างตัดหน้าคนละที่กับไฟล์จริง)
  {
    const f0 = P.mergeProseFormat({});
    check('★★ ย่อหน้าว่างกินที่เท่ากับย่อหน้าหนึ่งบรรทัด',
          P.proseBlockLines({ type: 'p', text: '' }, f0, 60)
            === P.proseBlockLines({ type: 'p', text: 'สั้น' }, f0, 60),
          P.proseBlockLines({ type: 'p', text: '' }, f0, 60));
  }
