...
// เพราะที่นั่นบรรทัดว่างเป็นเนื้อหาที่ paginate() นับเป็น 1 บรรทัดตั้งแต่ alpha.86)
{
  ...
  // ══ [alpha.132 ข้อ 1] ★★ กฎเดิมของฝั่งนิยายผิด — กลับด้านแล้ว ══
  // alpha.113 เว้นฝั่งนิยายไว้ด้วยเหตุผลว่า "นิยาย = มาร์กดาวน์ปกติ บรรทัดว่างเป็นแค่ตัวคั่นย่อหน้า"
  // แต่ **.md ของ Killian คือ "หนึ่งบรรทัด = หนึ่งบล็อก"**: `mdToDoc('ก\n\n\n\nข')` ให้ย่อหน้าว่าง
  // 3 ใบ และ `docToMd` เขียนกลับได้ตรงเป๊ะ → บรรทัดว่างในนิยายก็เป็นเนื้อหาจริงเท่ากับฝั่งบท
  // ผู้ใช้: *"pdf ออกมา บรรทัดว่างหาย"* — คืออาการของกฎเดิมข้อนี้ตรง ๆ
  check('[132-1] ★★ นิยาย: ช่วงบรรทัดว่างติดกันต้องอยู่ครบเหมือนฝั่งบท',
        /\n{4}/.test(pr) && nLines(pr) >= nLines(body) - 1,
        `compile ${nLines(pr)} บรรทัด · ต้นฉบับ ${nLines(body)}`);
  check('[132-1] ★ นิยายกับบทได้ช่องไฟชุดเดียวกันแล้ว (เนื้อฉากเดียวกัน)',
        pr.includes(body.trim()) && sp.includes(body.trim()));
  ...
  //  แล้วฉากบทฉากนั้นเสียบรรทัดว่างไปทั้งฉาก = PDF ตัดหน้าไม่ตรงกับที่เห็นบนจอ)
  {
    ...
    // [alpha.132 ข้อ 1] ฉากนิยายก็เก็บช่องไฟไว้แล้วเหมือนกัน — ทั้งเล่มใช้กฎเดียว ไม่ต้องแยกชนิด
    // (ก้อนสุดท้ายคือหางหลัง 'บรรยายท่อนสี่' ของฉากสุดท้าย — ไม่มีเนื้อฉากให้ตรวจ)
    check('[132-1] ★★ ฉากนิยายในเล่มเดียวกันก็เก็บช่องไฟไว้ครบเช่นกัน',
          parts.slice(1, -1).every((x) => /\n{4}/.test(x)),
          JSON.stringify(parts.slice(1, -1).join('|')));
    // แต่ **รอยต่อ** ที่โครงประกอบใส่เอง (หัวบท/หัวฉาก/สถิติ) ต้องไม่บวมเป็นบรรทัดว่างซ้อน
    {
      const wfHead = { id: 'h', name: 'h', ext: 'md',
                       steps: [{ key: 'chapter-heading', on: true },
                               { key: 'scene-heading', on: true }] };
      const proseOnly = { title: 'นิยายล้วน', author: '', chapters: [{ title: 'บทที่ 1', scenes: [
        { title: 'ฉากหนึ่ง', format: 'prose', body: 'ก', words: 1 },
        { title: 'ฉากสอง', format: 'prose', body: 'ข', words: 1 },
      ] }] };
      const th = CP.runWorkflow(proseOnly, wfHead).text;
      check('[132-1] ★ รอยต่อของโครงประกอบไม่บวมเป็นบรรทัดว่างซ้อน',
            !/\n{3,}/.test(th), JSON.stringify(th));
    }
  }
