...
{
  // [alpha.154] ป้ายชื่อของคำสั่งมาจากตัวรวบรวมของ commands-sync ตรง ๆ (CSV ไม่เก็บช่อง i18n_keys แล้ว)
  const LABEL_KEYS = new Set([...require('../tools/commands-sync.cjs').LABEL_KEYS].flatMap((k) => [k, 'ui.' + k]));
  const scIds = new Set(data.SHORTCUT_ROWS.map((r) => r.slice(3).join(':')));
  // ตัวอักษรที่เป็น "ไอคอนสำรอง" ในทะเบียน (icons/glyphs.csv) — ลูกศร/สามเหลี่ยมที่ใช้เป็นเครื่องหมายในประโยคไม่นับ
  const GLYPHS = new Set(Object.values(data.ICON_GLYPH).map((g) => String(g).replace(/️/g, '')));
  const iconGlyph = (g) => { const n = g.replace(/️/g, ''); return GLYPHS.has(n) && !'▸▾▼▲◀←↑→↓↔⇄'.includes(n); };
...
// ═══════════ 6b. [alpha.154] ทุกคำสั่งมีคำอธิบาย (hint) ในทุกภาษา ═══════════
// ผู้ใช้: *"เขียนคำอธิบาย hint ให้ครบทุกคำสั่งเลย"* — คำอธิบายอยู่ไฟล์ภาษาที่เดียว คีย์ `ui.tip.<command_id>`
// (tooltip.js ต่อเป็นบรรทัดที่สองใต้ชื่อ + คีย์ลัด · แผงใช้แถว `ui.tip.toggle-panel:<id>` เป็นคำอธิบายด้วย)
// คำสั่งใหม่ที่ไม่มีคำอธิบาย = เทสแดงตั้งแต่ยังไม่ build
{
  for (const f of fs.readdirSync(path.join(ROOT, 'languages')).filter((x) => /^k2_.+\.csv$/.test(x))) {
    const tbl = lexCsv(read('languages/' + f));
    const noTip = [...ids].filter((id) => !String(tbl['ui.tip.' + id] || '').trim());
    check(`★ ${f}: ทุกคำสั่งมีคำอธิบาย ui.tip.<id>`, noTip.length === 0, noTip.length + ' ขาด: ' + noTip.slice(0, 8).join(', '));
    const bad = Object.keys(tbl).filter((k) => k.startsWith('ui.tip.') && /\b(Ctrl|Alt|Shift|Cmd)\+/.test(tbl[k]));
    check(`${f}: คำอธิบายไม่พิมพ์คีย์ลัดตายตัว (หัวกล่องแสดงคีย์ลัดจริงให้แล้ว)`, bad.length === 0, bad.slice(0, 5).join(', '));
  }
}
