...
// [alpha.159 · M20] ตัวคั่นเป็นอักขระควบคุม \x01 (มองไม่เห็นในหน้าจอ แต่มีจริง) — คีย์ต้องไม่ชนกัน
// เมื่อขอบเขตของช่องเลื่อน · **ห้ามเปลี่ยนตัวคั่น** ไม่งั้นเครื่องหมายที่ผู้ใช้ติดไว้ในโปรเจกต์เก่าหายหมด
check('[159-M20] ★ ขอบเขตช่องต่างกัน = คีย์ต่างกัน (ไม่ต่อสตริงเปล่า)',
      N.navKey('ab', 'c', 'x', 0) !== N.navKey('a', 'bc', 'x', 0)
      && N.navKey('s', 'beat', 'x1', 0) !== N.navKey('s', 'beat', 'x', 10));
check('[159-M20] ตัวคั่นคงเดิม (\\x01) — คีย์ที่เก็บไว้ในโปรเจกต์เก่ายังใช้ได้',
      N.navKey('s1', 'beat', 'x', 0) === ['s1', 'beat', 'x', '0'].join(String.fromCharCode(1)));
}
