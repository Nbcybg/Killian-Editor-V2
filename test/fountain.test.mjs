import { parseScript, lineFor, prefixLen, suffixLen, stripFountainCodes } from '../src/fountain.js';
const md = [
  '. ตลาด - เย็น',
  'INT. บ้านโทระ - NIGHT',
  '!ลมพัดผ่านตลาดเก่า',
...
// ═══════════ [alpha.81r ข้อ 4] ตัดรหัส fountain ก่อนส่งออก ═══════════
// "ส่งออกต้องไม่มี code แบบ @ สิ มันโผล่มาได้ไง" — รหัสพวกนี้เป็นไวยากรณ์ของไฟล์ ไม่ใช่เนื้อเรื่อง
// ทางส่งออกที่ผ่าน parseScript (PDF บทหนัง/rtf/fdx) ไม่เจอปัญหา เพราะพาร์เซอร์กินรหัสไปแล้ว
// แต่ทางที่เป็น Markdown ล้วน (md/txt/html/PDF ของนิยาย) ไม่เคยมีใครตัดให้
{
  const strip = stripFountainCodes;
  t('ตัด @ หน้าชื่อตัวละคร', strip('@โทระ') === 'โทระ');
  t('ตัด . ที่ติดกับหัวฉากจริง', strip('.ตลาด - เย็น') === 'ตลาด - เย็น');
  t('ตัด > หน้าทรานซิชัน', strip('>CUT TO:') === 'CUT TO:');
  t('ตัด ! หน้าบรรยายบังคับ', strip('!ลมพัดผ่าน') === 'ลมพัดผ่าน');
  t('ตัดวงเล็บโน้ตทั้งหัวและท้าย', strip('((โน้ตถึงตัวเอง))') === 'โน้ตถึงตัวเอง');
  t('ตัด $shot', strip('$shot มุมกว้าง') === 'มุมกว้าง');
  t('ตัด = หน้าสรุป', strip('= โทระพบความลับ') === 'โทระพบความลับ');
  // สิ่งที่ห้ามแตะ — ไม่งั้นเนื้อเรื่องของผู้ใช้หายไปกับการส่งออก
  t('จุดที่มีวรรคตาม = ประโยคปกติ', strip('. ตลาด') === '. ตลาด');
  t('จุดไข่ปลาต้นบรรทัดไม่ถูกแตะ', strip('...เงียบไป') === '...เงียบไป');
  t('เลขทศนิยมต้นบรรทัดไม่ถูกแตะ', strip('.5 วินาที') === '.5 วินาที');
  t('รูป ![alt](src) ไม่ถูกแตะ', strip('![ปก](a.png)') === '![ปก](a.png)');
  t('อีเมลกลางบรรทัดไม่ถูกแตะ', strip('เมล a@b.com') === 'เมล a@b.com');
  t('บรรทัดที่มีแต่รหัส = ข้อความจริงของผู้ใช้', strip('@') === '@' && strip('.') === '.');
  t('ทำทั้งก้อนทีละบรรทัด',
    strip('@โทระ\nสวัสดี\n>CUT TO:') === 'โทระ\nสวัสดี\nCUT TO:');
  t('ค่าว่าง/undefined/null ไม่พัง',
    strip('') === '' && strip(undefined) === '' && strip(null) === '');
  // prefixLen/suffixLen ย้ายมาอยู่ไฟล์นี้แล้ว (markdown-code-toggle re-export ต่อ)
