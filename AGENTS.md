...
### ⚠️ กฎถาวร (alpha.76) — ห้ามเขียนข้อความไทยตายในโค้ดอีก

> **ทุกข้อความที่ผู้ใช้เห็นต้องแปลได้ · แปลจากไฟล์ CSV โดยไม่ต้อง build**

```js
setStatus('บันทึกแล้ว')            ✗   →   setStatus(T`บันทึกแล้ว`)            ✓
el('div', null, `บทที่ ${n}`)      ✗   →   el('div', null, T`บทที่ ${n}`)      ✓
```

- `T` = tagged template จาก **`src/i18n.js`** (โมดูลบริสุทธิ์ — โมดูลไหนก็ import ได้ รวมตัวที่มี unit test)
  · msgid = ประโยคไทยต้นฉบับ + `{0}`,`{1}` · ไม่มีคำแปล → คืนต้นฉบับ **ห้ามคืนค่าว่างเด็ดขาด**
- `t('dot.path')` ของเดิมยังใช้ได้ (`data-i18n` ใน index.html ใช้ตัวนี้) — ทั้งสองอ่านจากตารางเดียวกัน
- **ค่าที่ถูกเขียนลงไฟล์งาน ห้ามห่อ T``** (สถานะฉาก · คำนำหน้าหัวฉากของ fountain · ชนิดความสัมพันธ์ ·
  แท็กมาตรฐาน · คีย์ของ object) — เก็บเป็นไทยเสมอ แล้วแปล**ตอนแสดงผล**ด้วย `dataLabel(v)` จาก core.js
  ถ้าห่อ T`` ไฟล์เก่าจะอ่านไม่ออกทันทีที่ผู้ใช้เปลี่ยนภาษา
- **main.js เป็น CommonJS** → import `src/i18n.js` ไม่ได้ · มี `T` ตัวเล็กของตัวเองที่อ่าน CSV ก้อนเดียวกัน
  เพิ่มข้อความในเมนู OS ก็ห่อ T`` ได้ตามปกติ
- ค่าคงที่ระดับโมดูลห่อ T`` ได้ เพราะตารางถูกโหลด **แบบ synchronous** (`kapi.langSync`) ในบอดี้ของ
  `src/i18n.js` ซึ่ง esbuild วางไว้บนสุด — **ห้ามเปลี่ยนให้เป็น async** ไม่งั้นค่าพวกนั้นค้างภาษาเดิมทั้งรอบ

**ตรวจ/สกัดข้อความ** (รันหลังเพิ่มฟีเจอร์ทุกครั้ง):

```bash
npm run i18n              # ตรวจว่ามีข้อความไทยตกค้างกี่จุด อยู่ไหน (ไม่แก้ไฟล์)
npm run i18n:apply        # ห่อ T`` ให้อัตโนมัติ + อัปเดต languages/k2_*.csv
```

เครื่องมือมี 3 ด่านกันซอร์สพัง: ค่าจริงของ literal ทุกตัวต้องเท่าเดิม · esbuild ต้องพาร์สผ่าน ·
ต้องมี `import { T }` ครบ · ถ้าจะยกเว้นไฟล์/ช่วงบรรทัด เพิ่มใน `SKIP_FILES`/`SKIP_RANGES` ของ
`tools/i18n-extract.cjs` **พร้อมคอมเมนต์บอกเหตุผล**
...
- `tools/i18n-extract.cjs` (alpha.76) — ตรวจ/ห่อข้อความไทย + ออกไฟล์ `languages/k2_*.csv`
  (`npm run i18n` / `npm run i18n:apply`)
